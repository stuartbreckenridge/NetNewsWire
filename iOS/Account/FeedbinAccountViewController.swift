...
final class FeedbinAccountViewController: UITableViewController {
	...
	private func setupFooter() {
		footerLabel.text = Localizations.labelTextSignInToYourFeedbinAccountAndSyncYourFeedsAcrossYourDevicesYourUsernameAndPasswordWillBeEncryptedAndStoredInKeychainDontHaveAFeedbinAccount
	}
	...
	@IBAction func action(_ sender: Any) {
		guard let email = emailTextField.text, let password = passwordTextField.text else {
			showError(Localizations.labelTextUsernameAndPasswordRequired)
			return
		}
		...
		guard account != nil || !AccountManager.shared.duplicateServiceAccount(type: .feedbin, username: trimmedEmail) else {
			showError(Localizations.labelTextThereIsAlreadyAFeedbinAccountWithThatUsernameCreated)
			return
		}
		...
		Task { @MainActor in
			...
			do {
				...
				if let validatedCredentials {
					...
					showError(Localizations.labelTextInvalidEmailPasswordCombination)
				}
			} catch {
				stopAnimation()
				showError(Localizations.labelTextNetworkErrorTryAgainLater)
			}
		}
	}
	...
	private func showError(_ message: String) {
		presentError(title: Localizations.labelTextError, message: message)
	}
